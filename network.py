...
class Network:
    ...
    def __init__(self, arrival_rates, service_rates, routing_matrix):
        ...
        # For storing statistics over time
        self.stats = {
            'time': [],
            'num_jobs': [],
            'num_visits': [],
        ...
        self.server_stats = [[] for _ in range(len(self.routing_matrix))]
    ...
    def log_stats(self, current_time):
        ...
        self.stats['num_jobs'].append(Job.NUM_JOBS)
        self.stats['num_visits'].append(Job.NUM_VISITS)
        self.stats['time_spent'].append(Job.TIME_SPENT)
        for i in range(len(self.server_stats)):
            if self.num_samples > 1:
                new_avg = (self.server_stats[i][-1] * (self.num_samples - 1) + self.servers[i].num_jobs()) / self.num_samples
            else:
                new_avg = self.servers[i].num_jobs()
            self.server_stats[i].append(new_avg)

        if Job.NUM_JOBS > 0:
            self.stats['visits_per_job'].append(Job.NUM_VISITS / Job.NUM_JOBS)
            self.stats['time_per_job'].append(Job.TIME_SPENT / Job.NUM_JOBS)
        else:
            self.stats['visits_per_job'].append(0)
            self.stats['time_per_job'].append(0)
    ...
    def plot_stats(self):
        ...
        plt.subplot(321)
        plt.plot(self.stats['time'], self.stats['visits_per_job'], label='Visits per Job')
        plt.title('Average Visits per Job over Time')
        plt.xlabel('Time')
        plt.ylabel('Visits per Job')

        plt.subplot(322)
        plt.plot(self.stats['time'], self.stats['time_per_job'], label='Time Spent per Job')
        plt.title('Average Time Spent per Job over Time')
        plt.xlabel('Time')
        plt.ylabel('Time Spent per Job')

        for i in range(len(self.server_stats)):
            plt.subplot(323 + i)
            plt.plot(self.stats['time'], self.server_stats[i], label='Time Spent per Job')
            plt.title('Average No. of jobs in server ' + str(i+1))
            plt.xlabel('Time')
